...
echo "-----------------"
echo "BROKER_ACTIONS: Starting node at $BROKER_IP"

ssh -i "~/.ssh/$KEY_PAIR.pem" -o "StrictHostKeyChecking=no" -o "UserKnownHostsFile=/dev/null" ubuntu@$BROKER_IP sudo bash start.sh

echo "BROKER_ACTIONS: Starting node killed on $BROKER_IP"
echo "-----------------"