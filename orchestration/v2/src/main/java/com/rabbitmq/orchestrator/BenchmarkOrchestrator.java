...
public class BenchmarkOrchestrator {
    ...
    public void run(CmdArguments arguments) {
        ...
        for(Benchmark benchmark : playlist.getBenchmarks()) {
            boolean prepareSuccess = dispatcher.prepareSystems(benchmark);
            if(!prepareSuccess){
                LOGGER.warn("Terminating playlist run due to failure to prepare the systems for the benchmark");
                break;
            }
