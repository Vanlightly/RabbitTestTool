...
def teardown(technology, node, run_tag):
    terminated = False
...
def teardown_all(broker1_args_list, broker2_args_list, stop):
    ...
    for p in range(len(broker1_args_list)):
        ...
        for n in range(0, ba1.cluster_size):
            ...
            teardown(ba1.technology, str(node_num), run_tag)

        for n in range(0, ba2.cluster_size):
            node_num = int(ba2.node) + n
            teardown(ba2.technology, str(node_num), run_tag)
    print("All servers terminated")
    if stop:
        exit(1)

def run_benchmark(ba, sa, run_tag, topology):
    ...
    exit_code = subprocess.call(["bash", "run-logged-aws-benchmark.sh", ba.node, sa.key_pair, ba.technology, ba.broker_version, ba.instance, ba.volume, ba.filesystem, sa.hosting, ba.tenancy, sa.password, sa.postgres_url, sa.postgres_user, sa.postgres_pwd, topology, sa.run_id, broker_user, broker_password, run_tag, ba.core_count, ba.threads_per_core, ba.config_tag, str(ba.cluster_size), ba.no_tcp_delay, ba.policies_file, str(sa.override_step_seconds), str(sa.override_step_repeat),nodes])
    if exit_code != 0:
...
def parallel_deploy(broker1_args_list, broker2_args_list, sa, run_tag):
    global deploy_status
    ...
    for i in range(len(broker1_args_list)):
        ...
        if ba1.cluster_size == 1:
            ...
            if technology1 != "rabbitmq":
                print("Can only create clusters of RabbitMQ")
                teardown_all(broker1_args_list, run_tag, True)
        ...
        if ba2.cluster_size == 1:
            ...
            if technology2 != "rabbitmq":
                print("Can only create clusters of RabbitMQ")
                teardown_all(broker2_args_list, run_tag, True)
...
new_instance_per_topology = is_true(get_optional_arg(args, "--new-instance-per-topology", "false"))
...
BrokerArgs = namedtuple("BrokerArgs", "node technology broker_version cluster_size instance volume volume_size filesystem tenancy core_count threads_per_core no_tcp_delay config_tag vars_file policies_file")
...
for x in range(parallel_count):
    ...
    ba1_args = BrokerArgs(str(node_number1), technology1, version1, cluster_size1, instance1, volume1, volume_size1, fs1, tenancy1, core_count1, threads_per_core1, no_tcp_delay1, config_tag1, vars_file1, policies_file1)
    ...
    ba2_args = BrokerArgs(str(node_number2), technology2, version2, cluster_size2, instance2, volume2, volume_size2, fs2, tenancy2, core_count2, threads_per_core2, no_tcp_delay2, config_tag2, vars_file2, policies_file2)    
...
if len(topologies) > 0:
    ...
    if not new_instance_per_run and not new_instance_per_topology:
        run_tag = str(randint(1, 99999))
        parallel_deploy(ba1_list, ba2_list, sharedArgs, run_tag)
    ...
    for i in range(repeat_count):
        ...
        for topology in topologies:
            ...
            for p in range(parallel_count):
                ba1 = ba1_list[p]
                ba2 = ba2_list[p]
                t1 = threading.Thread(target=run_benchmark, args=(ba1, sharedArgs, run_tag, topology,))
                t2 = threading.Thread(target=run_benchmark, args=(ba2, sharedArgs, run_tag, topology,))
                b_threads.append(t1)
                b_threads.append(t2)
            ...
            for p in range(parallel_count):
                ...
                if benchmark_status[status_id1] != "success" or benchmark_status[status_id2] != "success":
                    ...
                    teardown_all(ba1_list, ba2_list, False)
                    exit(1)
